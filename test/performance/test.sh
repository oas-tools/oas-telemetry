...
node test "TLM LONG_30m TENS $i" $concurrentUsers $requests $requestDelay $problemSizeA_TENS $problemSizeB_TENS  && kill -9 `ps ax | grep node | grep -v grep | awk '{print $1}'`
...
node test "TLM LONG_30m HUNDREDS $i" $concurrentUsers $requests $requestDelay $problemSizeA_HUNDREDS $problemSizeB_HUNDREDS  && kill -9 `ps ax | grep node | grep -v grep | awk '{print $1}'`
